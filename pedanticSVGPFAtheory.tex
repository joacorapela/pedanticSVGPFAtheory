...
Consider the joint prior on observations, $Y$, latents
\begin{thm:variationalLowerBound}

    \begin{align}
        \log P(Y)\ge \mathcal{E}_{q{h_{nr}(\cdot))\left\{\log P(Y|h_{nr}(\cdot))\right\}-\sum_{k=1}^KKL(q(\mathbf{u}_{kr}||p(\mathbf{u}_{kr}))
    \begin{align}

\end{thm:variationalLowerBound}
